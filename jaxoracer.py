...
def steering_constraints(steering_angle, steering_velocity, p):
    clamped = jnp.where(
        ...
        p.v_min,
        jnp.where(steering_velocity >= p.v_max, p.v_max, steering_velocity),
    )
    at_limit = ((steering_angle <= p.min) & (steering_velocity <= 0)) | (
        (steering_angle >= p.max) & (steering_velocity >= 0)
    )
    return jnp.where(at_limit, 0.0, clamped)


def acceleration_constraints(velocity, acceleration, p):
    pos_limit = jnp.where(
        velocity > p.v_switch, p.a_max * p.v_switch / velocity, p.a_max
    )
    clamped = jnp.where(
        acceleration <= -p.a_max,
        -p.a_max,
        jnp.where(acceleration >= pos_limit, pos_limit, acceleration),
    )
    at_limit = ((velocity <= p.v_min) & (acceleration <= 0)) | (
        (velocity >= p.v_max) & (acceleration >= 0)
    )
    return jnp.where(at_limit, 0.0, clamped)
...
def vehicle_dynamics_st(x, uInit, p):
    ...
    def kinematic_branch(_):
        ...
        dd_psi = (
            ...
            / lwb
            * (
                u[1] * jnp.cos(x[6]) * jnp.tan(x[2])
                - x[3] * jnp.sin(x[6]) * d_beta * jnp.tan(x[2])
                + x[3] * jnp.cos(x[6]) * u[0] / jnp.cos(x[2]) ** 2
            )
        )
        return jnp.array([f_ks[0], f_ks[1], f_ks[2], f_ks[3], f_ks[4], dd_psi, d_beta])

    def dynamic_branch(_):
        return jnp.array(
            [
                x[3] * jnp.cos(x[6] + x[4]),
                x[3] * jnp.sin(x[6] + x[4]),
    ...
    return lax.cond(jnp.abs(x[3]) < 0.1, kinematic_branch, dynamic_branch, operand=None)
